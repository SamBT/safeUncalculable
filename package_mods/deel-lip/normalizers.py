...
def reshaped_kernel_orthogonalization(
    ...
    """
    if bound_only and (eps_bjorck is not None) and (beta is not None):
        print("Bad settings: you're trying to only bound k_lip, but have enabled bjorck algorithm")
        return None
    ...
    if bound_only:
        W_bar, u, sigma = spectral_bounding(
            W_reshaped, u, adjustment_coef, eps=eps_spectral, maxiter=maxiter_spectral
        )
    else:
    ...
    if bound_only:
        if sigma > adjustment_coef: # in this case, spectral_bounding normalized the kernel
            W_bar = W_bar * adjustment_coef
        else: # in this case, spectral bounding did nothing - shouldn't multiply by Lipschitz constant
            W_bar = W_bar
    else:
        W_bar = W_bar * adjustment_coef
...
def spectral_bounding(
    kernel, u, max_sigma, eps=DEFAULT_EPS_SPECTRAL, maxiter=DEFAULT_MAXITER_SPECTRAL
):
    """
    Normalize the kernel only if its spectral norm exceeds
    the desired Lipschitz coefficient, in which case normalize to 1
    and then it will be scaled to k_lip in reshaped_kernel_orthogonalization 
    """
