...
def construct_distributed_dense(input_tensor, sizes, acts='relu', k_inits='he_uniform', 
                                                                  names=None, l2_regs=0., spectral=False, lip_const=1.0, eps_bjorck=None, bound_only=False):
    """"""
    ...
    for s, act, k_init, name, l2_reg in zip(sizes, acts, k_inits, names, l2_regs):
        ...
        if spectral:
            d_layer = SpectralDense(s, k_coef_lip=lip_const, eps_bjorck=eps_bjorck, kernel_initializer=k_init, bound_only=bound_only, **kwargs)
        else:
...
class SymmetricPerParticleNN(NNBase):
    ...
    def _process_hps(self):
        ...
        **Default EFN Hyperparameters**
        ...
        self.spectral = self._proc_arg('spectral',default=False)
        self.lip_const = self._proc_arg('lip_const',default=1.0)
        self.eps_bjorck = self._proc_arg('eps_bjorck',default=None)
        self.bound_only = self._proc_arg('bound_only',default=False)
    ...
    def _construct_model(self):
        ...
        # get output layers
        if self.spectral:
            out_layer = SpectralDense(self.output_dim, k_coef_lip=self.lip_const, eps_bjorck=self.eps_bjorck, bound_only=self.bound_only, name=self._proc_name('output'))
        else:
    ...
    def _construct_Phi(self):
        ...
        Phi_layers, Phi_tensors = construct_distributed_dense(self.tensors[-1], self.Phi_sizes, 
                                                              ...
                                                              names=names, 
                                                              l2_regs=self.Phi_l2_regs,spectral=self.spectral,lip_const=self.lip_const,
                                                              eps_bjorck=self.eps_bjorck,bound_only=self.bound_only)
    ...
    def _construct_F(self):
        ...
        F_layers, F_tensors = construct_dense(self.tensors[-1], self.F_sizes,
                                              ...
                                              l2_regs=self.F_l2_regs,spectral=self.spectral,lip_const=self.lip_const,
                                              eps_bjorck=self.eps_bjorck,bound_only=self.bound_only)
