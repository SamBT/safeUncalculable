...
def construct_dense(input_tensor, sizes,
                    ...
                    names=None, spectral=False, lip_const=1.0, eps_bjorck=None, bound_only=False):
    """"""
    ...
    for s, act, k_init, dropout, l2_reg, name in z:
        ...
        kwargs = ({'kernel_regularizer': l2(l2_reg), 'bias_regularizer': l2(l2_reg)} 
                  if l2_reg > 0. else {})
        if spectral:
            dense_layer = SpectralDense(s, kernel_initializer=k_init, name=name, k_coef_lip=lip_const, eps_bjorck=eps_bjorck, bound_only=bound_only, **kwargs)
        else:
